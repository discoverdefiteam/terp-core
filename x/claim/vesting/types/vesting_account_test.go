...
import (
	...
	"github.com/terpnetwork/terp-core/x/claim/vesting/types"
)
...
func TestGetVestedCoinsPeriodicVestingAcc(t *testing.T) {
	...
	pva := types.NewTerpPeriodicVestingAccount(bacc, origCoins, periods)
...
func TestTerpPeriodicVestingAccountMarshal(t *testing.T) {
	baseAcc, coins := initBaseAccount()
	now := tmtime.Now()
	acc := types.NewTerpPeriodicVestingAccount(baseAcc, coins, types.Periods{types.Period{now.Unix(), 3600, coins, 0}})
	...
	require.Nil(t, err)
	require.IsType(t, &types.TerpPeriodicVestingAccount{}, acc2)
