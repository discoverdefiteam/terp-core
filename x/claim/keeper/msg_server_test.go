...
import (
	...
	"github.com/terpnetwork/terp-core/x/claim/keeper"
...
func (suite *KeeperTestSuite) TestSetAirdropAllocationsForMultiAirdrops() {
	...
	totalWeightTerp, err := suite.app.ClaimKeeper.GetTotalWeight(suite.ctx, types.DefaultAirdropIdentifier)
	suite.Require().NoError(err)
	suite.Require().Equal(totalWeightTerp, sdk.NewDecWithPrec(90, 2))

	totalWeightThiol, err := suite.app.ClaimKeeper.GetTotalWeight(suite.ctx, "juno")
	suite.Require().NoError(err)
	suite.Require().Equal(totalWeightThiol, sdk.NewDecWithPrec(50, 2))
