...
import (
	...
	"github.com/terpnetwork/terp-core/x/claim/types"
	vestingtypes "github.com/terpnetwork/terp-core/x/claim/vesting/types"
	epochstypes "github.com/terpnetwork/terp-core/x/epochs/types"
)
...
func (k Keeper) LoadAllocationData(ctx sdk.Context, allocationData string) bool {
	...
	for _, line := range lines {
		...
		terpAddr := utils.ConvertAddressToterpAddress(sourceChainAddr)
		if terpAddr == "" {
			continue
		}
		allocationIdentifier := airdropIdentifier + terpAddr
		...
		_, err = sdk.AccAddressFromBech32(terpAddr)
		if err != nil {
			continue
		}
...
func (k Keeper) GetUnallocatedUsers(ctx sdk.Context, identifier string, users []string, weights []sdk.Dec) ([]string, []sdk.Dec) {
	...
	for idx, user := range users {
		terpAddr := utils.ConvertAddressToterpAddress(user)
		addr, _ := sdk.AccAddressFromBech32(terpAddr)
...
func (k Keeper) GetUserVestings(ctx sdk.Context, addr sdk.AccAddress) (vestingtypes.Periods, sdk.Coins) {
	...
	terpVestingAcc, isterpVestingAccount := acc.(*vestingtypes.TerpPeriodicVestingAccount)
	if !isterpVestingAccount {
		return vestingtypes.Periods{}, sdk.Coins{}
	} else {
		return terpVestingAcc.VestingPeriods, terpVestingAcc.GetVestedCoins(ctx.BlockTime())
	}
}
...
func (k Keeper) ClaimCoinsForAction(ctx sdk.Context, addr sdk.AccAddress, action types.Action, airdropIdentifier string) (sdk.Coins, error) {
	...
	if !isPassed {
		...
		if !isterpVestingAccount {
			// Convert user account into terp veting account.
			...
			vestingAcc := vestingtypes.NewTerpPeriodicVestingAccount(baseAccount.(*authtypes.BaseAccount), claimableAmount, []vestingtypes.Period{{
			...
			// Grant a new vesting to the existing terp vesting account
			periodLength := GetAirdropDurationForAction(action)
			terpVestingAcc.AddNewGrant(vestingtypes.Period{
				StartTime:  ctx.BlockTime().Unix(),
				Length:     periodLength,
				Amount:     claimableAmount,
				ActionType: int32(action),
			})
			k.accountKeeper.SetAccount(ctx, terpVestingAcc)
		}
	}
