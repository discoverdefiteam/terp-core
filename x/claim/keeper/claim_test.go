...
import (
	...
	terpvestingtypes "github.com/terpnetwork/terp-core/x/claim/vesting/types"
)
...
func (suite *KeeperTestSuite) TestClaimAccountTypes() {
	...
	_, isVestingAcc := acc.(*terpvestingtypes.TerpPeriodicVestingAccount)
	...
	for _, addr := range []sdk.AccAddress{addr2, addr3, addr4} {
		_, err = suite.app.ClaimKeeper.ClaimCoinsForAction(suite.ctx, addr, types.ACTION_DELEGATE_STAKE, "stride")
		suite.Require().ErrorContains(err, "only BaseAccount and TerpPeriodicVestingAccount can claim")
	}
}
