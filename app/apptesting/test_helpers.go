...
type AppTestHelper struct {
	...
	TerpChain  *ibctesting.TestChain
...
func (s *AppTestHelper) Setup() {
	s.App = app.InitTerpTestApp(true)
	s.Ctx = s.App.BaseApp.NewContext(false, tmtypes.Header{Height: 1, ChainID: TerpChainID})
...
// Initializes a ibctesting coordinator to keep track of Terp and a host chain's state
func (s *AppTestHelper) SetupIBCChains(hostChainID string) {
	s.Coordinator = ibctesting.NewCoordinator(s.T(), 0)

	// Initialize a terp testing app by casting a TerpApp -> TestingApp
	ibctesting.DefaultTestingAppInit = app.InitTerpIBCTestingApp
	s.TerpChain = ibctesting.NewTestChain(s.T(), s.Coordinator, TerpChainID)
	...
	s.Coordinator.Chains = map[string]*ibctesting.TestChain{
		TerpChainID: s.TerpChain,
		hostChainID:   s.HostChain,
	}
	s.IbcEnabled = true
}

// Creates clients, connections, and a transfer channel between terp and a host chain
func (s *AppTestHelper) CreateTransferChannel(hostChainID string) {
	...
	s.TransferPath = NewTransferPath(s.TerpChain, s.HostChain)
	s.Coordinator.Setup(s.TransferPath)

	// Replace terp and host apps with those from TestingApp
	s.App = s.TerpChain.App.(*app.TerpApp)
	s.HostApp = s.HostChain.GetSimApp()
	s.Ctx = s.TerpChain.GetContext()

	// Finally confirm the channel was setup properly
	s.Require().Equal(ibctesting.FirstClientID, s.TransferPath.EndpointA.ClientID, "terp clientID")
	s.Require().Equal(ibctesting.FirstConnectionID, s.TransferPath.EndpointA.ConnectionID, "terp connectionID")
	s.Require().Equal(ibctesting.FirstChannelID, s.TransferPath.EndpointA.ChannelID, "terp transfer channelID")
...
func (s *AppTestHelper) CreateICAChannel(owner string) string {
	...
	icaPath := NewIcaPath(s.TerpChain, s.HostChain)
...
// Modifies sdk config to have terp address prefixes (used for non-keeper tests)
func SetupConfig() {
	app.SetupConfig()
}
